...
bool display_next_frame( )
{
	...
	// Create depth image
	auto depthFrame = _rs_camera->get_frame_data(rs::stream::depth);
	cv::Mat frameDepth(cv::Size(320, 240), CV_8UC1, (void*)depthFrame);
	...
	cv::FileStorage fp("open.xml", cv::FileStorage::WRITE);
	fp << "depth" << depth8u;
	fp.release();
	cvWaitKey( 1 );
	...
	imshow( WINDOW_RGB, frameDepth);
	imwrite("open.png",frameDepth);
	cvWaitKey( 1 );
